...
<head>
	...
	function delete_cat(cat_id)
	{
		if(confirm("确定删除该商家分类吗?")){
		  return document.merchant_cat_form.action="__URL__/deleteCat/id/"+cat_id;
		}else{
			return false;
		}
	}
...
<hr style="margin-right:10px;color:#EEEEEE;"/>
	<form method="post" action="<?php echo U('addCat');?>">
		<label for="name">大组名称:</label>
		<input type="text" id="input_name" name="name" style="min-width:180px;" required/>
		<input type="submit" value="添加"/>
	</form>
	<form name="merchant_cat_form">
		<table cellspacing="0" style="width:400px;">
			...
			<tbody>
				<?php if(is_array($merchantCats)): $i = 0; $__LIST__ = $merchantCats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td align="center"><?php echo ($vo['id']); ?></td>
						<td align="center"><a href="<?php echo U('cat_details');?>/catid/<?php echo ($vo['id']); ?>" target="main"><?php echo ($vo['name']); ?></a></td>
						<td align="center">
							<input type="submit" value="删除" onclick="delete_cat(<?php echo ($vo['id']); ?>)"/>
						</td>
